...
def bds_sampling_random(pc, num_sample, trans_num=np.e, sort='PCA', dim_reduction='PCA', order=None, start_index=0):

    n = pc.shape[0]

    assert start_index >= 0 and start_index < n - num_sample, "start_index must be in the range of [0, n - num_sample)"
    ...
    pc_sampled = pc_sorted[ranks[start_index:start_index+num_sample]]
...
def fps_feature(pc_feature, num_sample, random_seed=5):

    np.random.seed(random_seed)
    pc_samples_idx = fpsample.fps_npdu_kdtree_sampling(pc_feature, num_sample)

    return pc_feature[pc_samples_idx][:, :3]
